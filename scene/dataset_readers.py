...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    if eval:
        ...
        img_ids_train = [
            id_ for i, id_ in enumerate(img_ids) if files.loc[i, "split"] == "train"
        ]
        img_ids_test = [
            id_ for i, id_ in enumerate(img_ids) if files.loc[i, "split"] == "test"
        ]
...
def readNerfSyntheticInfo(
    path, white_background, eval, extension=".png", data_perturb=None
):
    ...
    train_cam_infos = readCamerasFromTransforms(
        ...
        split="train",
    )  # [CameraInfo(id，fov，R，T，图片路径，图片，高，宽)...100长度]
